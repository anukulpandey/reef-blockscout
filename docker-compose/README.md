...
```bash
cd ./docker-compose
FRONTEND_PORT=3000 docker compose -p reef-hosted-frontend -f reef-hosted-frontend.yml up -d
```
...
- The local compose file defaults `FRONTEND_PORT` to `0`, which tells Docker to choose any free host port. Set `FRONTEND_PORT=3000` when you want a fixed local port, and if Dokploy accidentally uses this file it will avoid failing on a `3000` bind conflict.
