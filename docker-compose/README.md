...
- Both hosted frontend compose files set `NEXT_PUBLIC_USE_NEXT_JS_PROXY=false` by default so the browser talks directly to the external backend URL you provide, instead of trying to reach a local `/node-api/proxy` endpoint.
- The local compose file defaults `FRONTEND_PORT` to `3009`, and the container itself also listens on that same port. If you override `FRONTEND_PORT`, the published host port and the internal Next.js port move together.
- The Dokploy compose file sets `HOSTNAME=0.0.0.0` and `PORT=3000` explicitly because the Next.js frontend image can fail to boot if it tries to bind using Docker's autogenerated container hostname.
